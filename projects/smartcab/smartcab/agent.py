...
class LearningAgent(Agent):
    ...
    def get_maxQ(self, state):
        ...
        maxQ = max(self.Q[stateHash].values())
        return maxQ 
    ...
    def createQ(self, state):
        ...
        stateHash = self.getStateHash(state)
        if (self.learning):
            if stateHash not in self.Q.keys():
                self.Q[stateHash] = dict()
                for a in self.valid_actions:
                    self.Q[stateHash][a] = 0.0
    ...
    def choose_action(self, state):
        ...
        if not self.learning:
            ...
            if self.epsilon > 0.01 and self.epsilon > random.random():
                ...
                for act in self.Q[stateHash]:
                    if maxQ == self.Q[stateHash][act]:
    ...
    def learn(self, state, action, reward):
        ...
        stateHash = self.getStateHash(state)
        if self.learning:
            self.Q[stateHash][action] = self.Q[stateHash][action] + self.alpha*(reward-self.Q[stateHash][action])
